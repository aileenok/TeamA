...
%% 10. Simulink decision command 생성
simulinkCmd = buildSimulinkDecisionCommand( ...
    bestCandidate, ...
    decision);

fprintf("\n=== Simulink Decision Command ===\n");
fprintf("Decision valid : %d\n", ...
    simulinkCmd.decisionValid);

fprintf("Action code    : %d\n", ...
    simulinkCmd.actionCode);

fprintf("Target d       : %.3f m\n", ...
    simulinkCmd.targetD);

fprintf("Target v       : %.3f m/s\n", ...
    simulinkCmd.targetV);

fprintf("Reference size : %d points\n", ...
    simulinkCmd.numRefPoints);
